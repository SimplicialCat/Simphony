...
(function() {
  ...
  function parseMusic(code) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (line === '') {
        if ((currentPart && currentPart.voices.length > 0) || currentVoiceArray.length > 0) {
          if (!currentPart) {
            currentPart = { voices: [] };
          }
          ...
          currentPart = null;  // 创建新part时设为null，下次使用时再创建
        }
        continue;
      }
    ...
    if (currentPart && currentPart.voices.length > 0) {
      parts.push(currentPart);
    }
    ...
    function parseVoiceLine(line, eventsArray, currentKey, edo, lineNumber) {
      ...
      // 分别设置12-EDO和19-EDO的八度偏移
      // 12-EDO: octaveOffset=0 → midi=0（C0）
      // 19-EDO: octaveOffset=2 → midi=38（C2）
      var defaultOctaveOffset = edo === 19 ? 2 : 0;

      console.log('parseVoiceLine: EDO=', edo, 'defaultOctaveOffset=', defaultOctaveOffset, 'lineNumber=', lineNumber);
    ...
    function parseTokens(tokens, pos, context, currentKey, eventsArray, edo, lineNumber) {
      ...
      console.log('parseTokens: currentOctave=', currentOctave, 'edo=', edo, 'lineNumber=', lineNumber);
      ...
      while (i < tokens.length) {
        ...
        if (noteInfo) {
          ...
          console.log('Note added:', noteInfo.pitch, 'midi=', noteInfo.midi, 'edo=', edo, 'lineNumber=', lineNumber);
    ...
    function parseNoteTokenWithContext(token, octaveOffset, key, edo) {
      ...
      console.log('parseNoteTokenWithContext: token=', token, 'octaveOffset=', octaveOffset, 'localOctaveOffset=', localOctaveOffset, 'totalOctaveOffset=', totalOctaveOffset, 'key=', key);
  ...
  AudioPlayer.prototype.schedule = function(startFromSec) {
    ...
    console.log('[schedule] Total parts:', this.score.parts.length);
    var bpm = this.score.bpm;
    var scheduleStartTime = this.audioCtx.currentTime;  // 固定调度开始时间
    this.startTime = scheduleStartTime - startFromSec;

    var currentPartStartBeat = 0;
    var player = this;

    // 在schedule开始时设置isPlaying=true，确保timeout回调能执行
    var wasPlaying = this.isPlaying;
    this.isPlaying = true;

    console.log('[schedule] isPlaying set to true (was:', wasPlaying, ')');
    console.log('[schedule] scheduleStartTime=', scheduleStartTime.toFixed(3), 'startTime=', this.startTime.toFixed(3));

    for (var partIdx = 0; partIdx < this.score.parts.length; partIdx++) {
      var part = this.score.parts[partIdx];
      console.log('[schedule] Part', partIdx, 'has', part.voices.length, 'voices');

      for (var v = 0; v < part.voices.length; v++) {
        var voice = part.voices[v];
        console.log('[schedule] Part', partIdx, 'Voice', v, 'has', voice.length, 'events');
        for (var e = 0; e < voice.length; e++) {
          var evt = voice[e];
          if (evt.type !== 'rest') {
            console.log('[schedule] Part', partIdx, 'Voice', v, 'Event', e, 'lineNumber=' + evt.lineNumber, 'type=' + evt.type, 'pitch=' + (evt.pitch || 'N/A'));
          }
          ...
          if (this.onLineHighlight && evt.lineNumber) {
            var lineDelay = (adjustedStart - (scheduleStartTime - player.startTime)) * 1000;
            console.log('[schedule] Event lineNumber=' + evt.lineNumber + ' lineDelay=' + lineDelay.toFixed(2) + 'ms adjustedStart=' + adjustedStart.toFixed(2));

            // 使用IIFE创建独立的作用域，捕获当前evt的值
            (function(capturedEvt, capturedAdjustedStart, capturedDurationSec) {
              var lineTimeout = setTimeout(function() {
                console.log('[onLineHighlight timeout fired] lineNumber=' + capturedEvt.lineNumber + ' isPlaying=' + player.isPlaying + ' isPaused=' + player.isPaused);
                if (player.isPlaying && !player.isPaused) {
                  player.onLineHighlight(capturedEvt.lineNumber);
                }
              }, Math.max(0, lineDelay));
              player.activeTimeouts.push(lineTimeout);

              // 在音符结束时清除高亮
              if (player.onLineClear && capturedEvt.lineNumber) {
                var endDelay = (capturedAdjustedStart + capturedDurationSec - (scheduleStartTime - player.startTime)) * 1000;
                var endTimeout = setTimeout(function() {
                  if (!player.isPaused) {
                    player.onLineClear(capturedEvt.lineNumber);
                  }
                }, Math.max(0, endDelay));
                player.activeTimeouts.push(endTimeout);
              }
            })(evt, adjustedStart, durationSec);
          }
  ...
  AudioPlayer.prototype.scheduleNote = function(midi, startTime, durationSec) {
    // 对于19-EDO，音频midi提高两个八度，但显示midi保持不变
    var audioMidi = midi;
    var displayMidi = midi;

    if (this.score.edo === 19) {
      audioMidi = midi + 38;  // 提高两个八度用于音频（19 * 2）
    }

    var freq = edoToFreq(audioMidi, this.score.edo || 12);
    if (!freq) return;

    // 调用音符开始回调（使用displayMidi用于键盘显示）
    if (this.onNoteStart) {
      var player = this;
      var noteOnDelay = (startTime - player.audioCtx.currentTime) * 1000;
      var noteOnTimeout = setTimeout(function() {
        if (player.isPlaying && !player.isPaused) {
          player.onNoteStart(displayMidi);  // 使用displayMidi
        }
      }, Math.max(0, noteOnDelay));
      if (this.activeTimeouts) {
        this.activeTimeouts.push(noteOnTimeout);
      }
    }
    ...
    if (this.onNoteEnd) {
      ...
      var noteOffTimeout = setTimeout(function() {
        if (player.isPlaying && !player.isPaused) {
          player.onNoteEnd(displayMidi);  // 使用displayMidi
        }
      }, Math.max(0, noteOffDelay));
      if (this.activeTimeouts) {
        this.activeTimeouts.push(noteOffTimeout);
      }
    }
  ...
  AudioPlayer.prototype.stopAllNotes = function() {
    console.log('[stopAllNotes] Clearing ' + (this.activeTimeouts ? this.activeTimeouts.length : 0) + ' timeouts');
    console.log('[stopAllNotes] Call stack:', new Error().stack);
    // 清除所有排队的timeout
    if (this.activeTimeouts) {
  ...
  AudioPlayer.prototype.play = function() {
    console.log('[AudioPlayer.play] Creating new play session');
    this.initAudio();
